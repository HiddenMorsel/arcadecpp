    ...
    std::vector<Contact> search(std::string pattern) {
      std::vector<Contact> contatosBuscados;
        ...
        for(int i = 0; i < (int) contatos.size(); i++){
          ...
          // std::string nomeContato = contatos[i].getName();
          std::vector<Fone> fones = contatos[i].getFones();
          if( contatos[i].getName().find(pattern) || fones[i].getId().find(pattern) ||  fones[i].getNumber().find(pattern))
